...
float ColorRGB::max() { return std::max(r, std::max(g, b)); }
...
int ColorRGB::r255() const { return static_cast<int>(std::floor(r * 255.0)); }

int ColorRGB::g255() const { return static_cast<int>(std::floor(g * 255.0)); }

int ColorRGB::b255() const { return static_cast<int>(std::floor(b * 255.0)); }

float ColorRGB::sum() { return r + g + b; }
