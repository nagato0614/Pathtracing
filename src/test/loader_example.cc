...
public:
		...
		time_t sec()
		{ return static_cast<time_t>(tv[1].tv_sec - tv[0].tv_sec); }
		time_t msec()
		{
		...
		for (size_t f = 0; f < shapes[i].mesh.num_face_vertices.size(); f++) {
			...
			for (size_t v = 0; v < fnum; v++) {
				...
				printf("    face[%ld].v[%ld].idx = %d/%d/%d\n", static_cast<long>(f),
							 ...
							 idx.texcoord_index);
				printf("    face[%ld].v[%ld].vertex_point = (%f, %f, %f)\n",
							 static_cast<long>(f),
							 static_cast<long>(v),
...
int main(int argc, char **argv)
{
	if (argc > 1) {
		...
		TestLoadObj(argv[1], basepath);
//    assert(TestLoadObj(argv[1], basepath));
	}
	else {
//     assert(TestLoadObj("cornell_box.obj"));
		// assert(true == TestLoadObj("cube.obj"));
//    assert(TestStreamLoadObj());
		TestStreamLoadObj();
//    assert(TestLoadObj("models/catmark_torus_creases0.obj", "models/", false));
	}
