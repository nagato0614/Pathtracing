...
std::tuple<Vector3, Vector3> tangentSpace(const Vector3 &n)
{
  ...
  return {Vector3(1 + s * n.x * n.x * a, s * b, -s * n.x), Vector3(b, s + n.y * n.y * a, -n.y)};
}

int tonemap(float v)
{
  return std::min(std::max(int(std::pow(v / (1.0 + v), 1 / 2.2) * 255), 0), 255);
}

int clamp(float v) { return std::min(std::max(0, int(v)), 255); }
              ...
              Spectrum zbar)
{
  ...
  for (const auto &i : s)
  {
    ...
    ofs << pixelColor.r255() << " " << pixelColor.g255() << " " << pixelColor.b255() << "\n";
  }
}

float linearInterpolate(float t, float a, float b) { return (1.0f - t) * a + t * b; }
